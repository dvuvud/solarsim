...
namespace solarsim {
	...
	Engine::Engine(unsigned int width, unsigned int height, const std::string& title)
	{
		...
		m_simulation->spawnEntity(std::make_unique<Planet>(glm::vec3(0.f,1.f,-25.f), 1.f, 1.f));
		m_simulation->spawnEntity(std::make_unique<Sun>(glm::vec3(0.f,1.f,0.f), 100.f, 1.f));
	}
